...
#include <csignal>
//modify by ff #include <gperftools/heap-profiler.h>
//modify by ff #include <third_party/lss/linux_syscall_support.h>
...
//modify by ff #include "exprs/timezone_db.h"
#include "gutil/atomicops.h"
#include "gutil/strings/substitute.h"
//modify by ff #include "rpc/authentication.h"
//modify by ff #include "rpc/thrift-util.h"
...
//modify by ff #include "util/jni-util.h"
#include "util/logging-support.h"
#include "util/mem-info.h"
//modify by ff #include "util/memory-metrics.h"
...
#include "util/time.h"
//modify by ff #include "util/zip-util.h"
...
[[noreturn]] static void LogMaintenanceThread() {
  ...
  while (true) {
    ...
    // to the process.
  //modify by ff  impala::CheckAndRotateMinidumps(FLAGS_max_minidumps);
...
[[noreturn]] static void MemoryMaintenanceThread() {
  while (true) {
    ...
    // somewhat up-to-date.
    //modify by ff AggregateMemoryMetrics::Refresh();
  }
}
...
/*modify by ff    ShutdownStatusPB shutdown_status;
...
[[noreturn]] static void HandleSigTerm(int signum, siginfo_t* info, void* context) {
 ...
 //modify by ff sys_write(STDOUT_FILENO, msg, strlen(msg));
#ifdef CODE_COVERAGE_ENABLED
...
//modify by ff  ABORT_IF_ERROR(GetHostname(&FLAGS_hostname));
...
# else
  ...
  if (!external_fe) {
  //modify by ff   ABORT_IF_ERROR(RegisterMinidump(argv[0]));
  }
#ifndef THREAD_SANITIZER
#ifndef __aarch64__
//modify by ff  AtomicOps_x86CPUFeaturesInit();
#endif
#endif
  impala::InitThreading();
  impala::datetime_parse_util::SimpleDateFormatTokenizer::InitCtx();
//modify by ff  impala::SeedOpenSSLRNG();
 //modify by ff ABORT_IF_ERROR(impala::InitAuth(argv[0]));
  ...
  //modify by ff InitThriftLogging();

  LOG(INFO) << CpuInfo::DebugString();
  LOG(INFO) << DiskInfo::DebugString();
  //modify by ff LOG(INFO) << MemInfo::DebugString();
  LOG(INFO) << OsInfo::DebugString();
  //modify by ff LOG(INFO) << CGroupUtil::DebugString();
  LOG(INFO) << "Process ID: " << getpid();
 //modify by ff LOG(INFO) << "Default AES cipher mode for spill-to-disk: "
 //           << EncryptionKey::ModeToString(EncryptionKey::GetSupportedDefaultMode());
  ...
  if (FLAGS_use_resolved_hostname) {
    IpAddr ip_address;
/*modify by ff    Status status = HostnameToIpAddr(FLAGS_hostname, &ip_address);
...
//modify by ff  ABORT_IF_ERROR(impala::LibCache::Init(external_fe));
/*modify by ff  Status fs_cache_init_status = impala::HdfsFsCache::Init();
...
//modify by ff  if (impala::KuduIsAvailable()) impala::InitKuduLogging();

#if !defined(ADDRESS_SANITIZER) && !defined(THREAD_SANITIZER)
  // tcmalloc and address sanitizer can not be used together
  if (FLAGS_enable_process_lifetime_heap_profiling) {
//modify by ff    HeapProfilerStart(FLAGS_heap_profile_dir.c_str());
  }
#endif
  ...
  if (external_fe || test_mode == TestInfo::FE_TEST) {
   ...
   //modify by ff ABORT_IF_ERROR(TimezoneDatabase::Initialize());
  }
}

Status impala::StartMemoryMaintenanceThread() {
//modify by ff  DCHECK(AggregateMemoryMetrics::TOTAL_USED != nullptr) << "Mem metrics not registered.";
