...
//modify by ff #include "kudu/util/flags.h"
...
*/
  ...
  for (const auto& flag: flags) {
    if (flag.hidden) {
//modify by ff      string flag_value = CheckFlagAndRedact(flag, kudu::EscapeMode::NONE);
//modify by ff      ss << "--" << flag.name << "=" << flag_value << "\n";
    }
  }
...
void impala::CheckAndRotateLogFiles(int max_log_files) {
  ...
  for (int severity = 0; severity < google::NUM_SEVERITIES; ++severity) {
    ...
    //modify by ff impala::LoggingSupport::DeleteOldLogs(fname, max_log_files);
  }
}
