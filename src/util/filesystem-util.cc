...
//modify by ff  const string& kernel_release = kudu::Env::Default()->GetKernelRelease();
  const string& kernel_release = nullptr;
  autodigit_less lt;
...
Status FileSystemUtil::CheckForBuggyExtFS(const string& path) {
  bool is_on_ext;
/*modify by ff  KUDU_RETURN_IF_ERROR(kudu::Env::Default()->IsOnExtFilesystem(path, &is_on_ext),
