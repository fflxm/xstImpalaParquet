...
/*modify by ff  const int encode_result = sasl_encode64(in, static_cast<unsigned>(in_len), out,
...
bool Base64Decode(const char* in, int64_t in_len, int64_t out_max, char* out,
  ...
  if (UNLIKELY((in_len & 3) != 0)) return false;
/*modify by ff  const int decode_result = sasl_decode64(in, static_cast<unsigned>(in_len), out,
