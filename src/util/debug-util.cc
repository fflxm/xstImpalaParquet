...
//modify by ff PRINT_THRIFT_ENUM_IMPL(QueryState)
PRINT_THRIFT_ENUM_IMPL(Encoding)
//modify by ff PRINT_THRIFT_ENUM_IMPL(TCatalogObjectType)
//modify by ff PRINT_THRIFT_ENUM_IMPL(TCatalogOpType)
//modify by ff PRINT_THRIFT_ENUM_IMPL(TDdlType)
...
//modify by ff PRINT_THRIFT_ENUM_IMPL(TImpalaQueryOptions)
...
string GetBuildVersion(bool compact) {
  stringstream ss;
//modify by ff  ss << GetDaemonBuildVersion()
ss << "ff"
#ifdef NDEBUG
     << " RELEASE"
#else
     << " DEBUG"
#endif
//modify by ff      << " (build " << GetDaemonBuildHash()
     << ")";
  if (!compact) {
//modify by ff     ss << endl << "Built on " << GetDaemonBuildTime();
  }
  return ss.str();
}
