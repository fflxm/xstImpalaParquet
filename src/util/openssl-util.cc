...
//modify by ff  return !FLAGS_ssl_client_ca_certificate.empty() &&
...
//modify by ff  return !FLAGS_ssl_server_certificate.empty() && !FLAGS_ssl_private_key.empty();
  return true;
}
...
//modify by ff      return (CpuInfo::IsSupported(CpuInfo::PCLMULQDQ)
