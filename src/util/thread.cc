...
//   { ... }
// ]
//modify by ff void JvmThreadsUrlCallback(const Webserver::WebRequest& req, Document* doc);
/*//modify by ff
...
class ThreadMgr {
 public:
  ...
  //             }
  //        ]
//modify by ff  void ThreadGroupUrlCallback(const Webserver::WebRequest& req, Document* output);
...
void ThreadMgr::GetThreadOverview(Document* document) {
  lock_guard<mutex> l(lock_);
/*//modify by ff  if (metrics_enabled_) {
