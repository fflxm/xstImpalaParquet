...
//modify by ff #include "kudu/util/path_util.h"
...
string GetHdfsErrorMsg(const string& prefix, const string& file) {
  ...
  stringstream ss;
  ss << prefix << file << "\n" << error_msg;
  /*modify by ff char* root_cause = hdfsGetLastExceptionRootCause();
...
/*modify by ff  hdfsFileInfo* info = hdfsGetPathInfo(connection, filename);
...
//modify by ff  int error = hdfsCopy(src_conn, src_path.c_str(), dst_conn, dst_path.c_str());
  int error = 0;
  if (error != 0) {
