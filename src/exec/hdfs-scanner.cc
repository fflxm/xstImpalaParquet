...
/*modify by ff    bool is_sequence_based = BaseSequenceScanner::FileFormatIsSequenceBased(
...
/*modify by ff  return CallCodegendOrInterpreted<WriteTuplesFn>::invoke(this, write_tuples_fn_,
...
Status HdfsScanner::IssueFooterRanges(HdfsScanNodeBase* scan_node,
    ...
    int64_t footer_size_estimate,bool blocal) {
  DCHECK(!files.empty());
  ...
  if (footer_ranges.size() > 0) {
    if (blocal){
      ...
      RETURN_IF_ERROR(scan_node->AddDiskIoRangesLocal(footer_ranges, EnqueueLocation::TAIL));
    }
  }
  return Status::OK();
}
