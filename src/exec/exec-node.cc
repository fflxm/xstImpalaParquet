...
//modify by ff #include "codegen/codegen-anyval.h"
//modify by ff #include "codegen/llvm-codegen.h"
#include "common/object-pool.h"
#include "common/status.h"
//modify by ff #include "exec/aggregation-node.h"
//modify by ff #include "exec/analytic-eval-node.h"
//modify by ff #include "exec/cardinality-check-node.h"
//modify by ff #include "exec/data-source-scan-node.h"
//modify by ff #include "exec/empty-set-node.h"
//modify by ff #include "exec/exchange-node.h"
#include "exec/exec-node-util.h"
//modify by ff #include "exec/hbase-scan-node.h"
//modify by ff #include "exec/hdfs-scan-node-mt.h"
#include "exec/hdfs-scan-node.h"
//modify by ff #include "exec/kudu-scan-node-mt.h"
//modify by ff #include "exec/kudu-scan-node.h"
//modify by ff #include "exec/kudu-util.h"
//modify by ff #include "exec/nested-loop-join-node.h"
//modify by ff #include "exec/partial-sort-node.h"
//modify by ff #include "exec/partitioned-hash-join-node.h"
//modify by ff #include "exec/select-node.h"
//modify by ff #include "exec/singular-row-src-node.h"
//modify by ff #include "exec/sort-node.h"
//modify by ff #include "exec/streaming-aggregation-node.h"
//modify by ff #include "exec/subplan-node.h"
//modify by ff #include "exec/topn-node.h"
//modify by ff #include "exec/union-node.h"
//modify by ff #include "exec/unnest-node.h"
...
//modify by ff  codegen_status_msgs_.emplace_back(FragmentState::GenerateCodegenMsg(
...
//modify by ff      *node = pool->Add(new AggregationPlanNode());
      break;
    case TPlanNodeType::HASH_JOIN_NODE:
//modify by ff      *node = pool->Add(new PartitionedHashJoinPlanNode());
      break;
    case TPlanNodeType::NESTED_LOOP_JOIN_NODE:
//modify by ff      *node = pool->Add(new NestedLoopJoinPlanNode());
      break;
    case TPlanNodeType::EMPTY_SET_NODE:
 //modify by ff     *node = pool->Add(new EmptySetPlanNode());
      break;
    case TPlanNodeType::EXCHANGE_NODE:
//modify by ff      *node = pool->Add(new ExchangePlanNode());
      break;
    case TPlanNodeType::SELECT_NODE:
//modify by ff      *node = pool->Add(new SelectPlanNode());
      break;
    case TPlanNodeType::SORT_NODE:
      if (tnode.sort_node.type == TSortType::PARTIAL) {
 //modify by ff       *node = pool->Add(new PartialSortPlanNode());
      } else if (tnode.sort_node.type == TSortType::TOPN ||
          tnode.sort_node.type == TSortType::PARTITIONED_TOPN) {
//modify by ff        *node = pool->Add(new TopNPlanNode());
      } else {
        DCHECK(tnode.sort_node.type == TSortType::TOTAL);
 //modify by ff       *node = pool->Add(new SortPlanNode());
      }
      break;
    case TPlanNodeType::UNION_NODE:
//modify by ff      *node = pool->Add(new UnionPlanNode());
      break;
    case TPlanNodeType::ANALYTIC_EVAL_NODE:
//modify by ff      *node = pool->Add(new AnalyticEvalPlanNode());
      break;
    case TPlanNodeType::SINGULAR_ROW_SRC_NODE:
//modify by ff      *node = pool->Add(new SingularRowSrcPlanNode());
      break;
    case TPlanNodeType::SUBPLAN_NODE:
//modify by ff      *node = pool->Add(new SubplanPlanNode());
      break;
    case TPlanNodeType::UNNEST_NODE:
//modify by ff      *node = pool->Add(new UnnestPlanNode());
      break;
    case TPlanNodeType::CARDINALITY_CHECK_NODE:
//modify by ff      *node = pool->Add(new CardinalityCheckPlanNode());
      break;
    default:
...
//modify by ff  DCHECK(state->codegen() != nullptr);
...
// }
//
/*modify by ff Status ExecNode::CodegenEvalConjuncts(LlvmCodeGen* codegen,
  ...
  //modify by ff llvm::PointerType* tuple_row_ptr_type = codegen->GetStructPtrType<TupleRow>();
  llvm::PointerType* tuple_row_ptr_type = nullptr;
