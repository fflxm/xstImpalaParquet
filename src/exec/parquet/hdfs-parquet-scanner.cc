...
/*modify by ff #include "exprs/literal.h"
...
Status HdfsParquetScanner::IssueInitialRangesLocal(HdfsScanNodeBase* scan_node,
    const vector<HdfsFileDesc*>& files, bool blocal) {
  DCHECK(!files.empty());
  ...
  return IssueFooterRanges(
      scan_node, THdfsFileFormat::PARQUET, files, PARQUET_FOOTER_SIZE, blocal);
}
...
Status HdfsParquetScanner::Open(ScannerContext* context) {
  ...
  /*modify by ff  if(expr.GetSlotIds(&slot_ids) == 1) {
...
/*modify by ff  if (state_->query_options().parquet_bloom_filtering) {
...
//modify by ff    conjuncts[conjunct_idx]->GetSlotIds(&conjunct_slot_ids_);
  }
}
...
Status HdfsParquetScanner::EvaluateStatsConjuncts(
  ...
  *skip_row_group = false;
  ...
  for (int i = 0; i < stats_conjunct_evals_.size(); ++i) {
    ...
    //modify by ff const string& fn_name = eval->root().function_name();
    const string& fn_name = nullptr;
...
/*modify by ff    VLOG(3) << "A filter is determined to be not useful:"
            << " fid=" << filter_id
  ...
  for (ScalarExprEvaluator* eval : stats_conjunct_evals_) {
    ...
    if (function_name == "le" || function_name == "ge") {
      ...
      if (match_found) {
        ...
        uint8_t* ptr = nullptr;
        size_t len = -1;
/*modify by ff        RETURN_IF_ERROR(LiteralToParquetType(*child_literal, eval,
/*modify by ff              node->element->type, &buffer, &ptr, &len));
        DCHECK(len != -1);
...
Status HdfsParquetScanner::InitScalarColumns() {
  ...
  for (BaseScalarColumnReader* scalar_reader : scalar_readers_) {
    ...
    //modify by ff
    dynamic_cast<ParquetColumnReader *>(scalar_reader)->setTupleOffset(col_chunk.meta_data.data_page_offset);
    scan_node_->tuple_desc()->slots()[i]->setTupleOffset(col_chunk.meta_data.data_page_offset);
    i++;
