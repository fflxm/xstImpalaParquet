...
//modify by ff       *node = pool->Add(new HBaseScanNode(pool, *this, state->desc_tbl()));
      break;
    case TPlanNodeType::DATA_SOURCE_NODE:
//modify by ff      *node = pool->Add(new DataSourceScanNode(pool, *this, state->desc_tbl()));
      break;
    case TPlanNodeType::KUDU_SCAN_NODE:
/*modify by ff      if (tnode_->kudu_scan_node.use_mt_scan_node) {
...
//modify by ff    filter_ctx.stats = state->obj_pool()->Add(new FilterStats(profile));
  }
