...
//modify by ff #include "kudu/util/env.h"
...
//modify by ff #include "util/kudu-status-util.h"
...
Status TmpDirS3::VerifyAndCreate(MetricGroup* metrics, vector<bool>* is_tmp_dir_on_disk,
  ...
  DCHECK(is_parsed_);
  hdfsFS hdfs_conn;
//modify by ff  RETURN_IF_ERROR(HdfsFsCache::instance()->GetConnection(
...
/*//modify by ff  RWFileOptions opts;
...
/*modify by ff  Status status = HdfsFsCache::instance()->GetConnection(
