...
/*modify by ff #include "kudu/rpc/rpc_context.h"
...
//modify by ff  host_profile_->GetCounters(
...
//modify by ff          fis->GetStatusReport(
//modify by ff              instance_status, nullptr, agg_profile, report_overall_status);
        } else {
          profiles_forest->profile_trees.emplace_back();
//modify by ff          fis->GetStatusReport(instance_status, &profiles_forest->profile_trees.back(),
//modify by ff              nullptr, report_overall_status);
        }
      }
...
/*modify by ff  if (UNLIKELY(!serialize_status.ok() ||
...
//modify by ff  RpcController rpc_controller;
 ...
 /*modify by ff   kudu::faststring sidecar_buf;
...
//modify by ff  rpc_controller.set_timeout(
//modify by ff      MonoDelta::FromMilliseconds(FLAGS_backend_client_rpc_timeout_ms));
  ReportExecStatusResponsePB resp;
//modify by ff  rpc_status = FromKuduStatus(proxy_->ReportExecStatus(report, &resp, &rpc_controller),
//modify by ff      "ReportExecStatus() RPC failed");
...
//modify by ff  ExecEnv::GetInstance()->stream_mgr()->Cancel(query_id());
}
/*modify by ff
