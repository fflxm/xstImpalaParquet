...
/*modify by ff  RETURN_IF_ERROR(DataSinkConfig::CreateConfig(
...
//modify by ff  if (codegen_ != nullptr) codegen_->Close();
...
/*modify by ff  if (codegen_.get() != NULL) return Status::OK();
...
/*modify by ff  for (auto& item : scalar_exprs_to_codegen_) {
    llvm::Function* fn;
