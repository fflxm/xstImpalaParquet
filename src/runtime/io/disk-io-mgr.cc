...
/*modify by ff  hdfsFile remote_hdfs_file =
...
//modify by ff      ret = hdfsWrite(hdfs_conn, remote_hdfs_file, buffer, bytes);
    }
    if (ret == -1 || ret != bytes) {
      queue->write_io_err()->Increment(1);
      status = Status(TErrorCode::DISK_IO_ERROR, GetBackendString(),
...
/*modify by ff  if (remote_hdfs_file != nullptr) {
...
DiskIoMgr::DiskIoMgr() :
    ...
    min_buffer_size_(BitUtil::RoundDownToPowerOfTwo(FLAGS_min_buffer_size))/*modify by ff,
...
DiskIoMgr::DiskIoMgr(int num_local_disks, int threads_per_rotational_disk,
    ...
    min_buffer_size_(BitUtil::RoundDownToPowerOfTwo(min_buffer_size))/*modify by ff,
    file_handle_cache_(min(FLAGS_max_cached_file_handles,
        FileSystemUtil::MaxNumFileHandles()),
        FLAGS_num_file_handle_cache_partitions,
        FLAGS_unused_file_handle_timeout_sec, &hdfs_monitor_)*/ {
...
//modify by ff  if (cached_read_options_ != nullptr) hadoopRzOptionsFree(cached_read_options_);
//modify by ff  if (remote_data_cache_) remote_data_cache_->ReleaseResources();
}
...
//modify by ff  RETURN_IF_ERROR(hdfs_monitor_.Init(disk_thread_group_.Size()));
//modify by ff   RETURN_IF_ERROR(file_handle_cache_.Init());
