...
//modify by ff #include "gen-cpp/data_stream_service.proxy.h"
...
//modify by ff #include "util/bloom-filter.h"
...
//modify by ff  const kudu::Status controller_status = rpc_controller->status();
...
/*modify by ff    RpcController* controller = obj_pool_.Add(new RpcController);
...
//modify by ff    proxy->UpdateFilterAsync(params, res, controller,
...
void RuntimeFilterBank::Close() {
  ...
  closed_ = true;
  for (auto& entry : filters_) {
//modify by ff    for (BloomFilter* filter : entry.second->bloom_filters) filter->Close();
