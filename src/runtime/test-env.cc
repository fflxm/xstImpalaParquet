...
/*modify by ff  exec_env_->tmp_file_mgr_.reset(new TmpFileMgr);
  ...
  IpAddr ip_address;
//modify by ff  RETURN_IF_ERROR(HostnameToIpAddr(FLAGS_hostname, &ip_address));
  exec_env_->krpc_address_.__set_hostname(ip_address);
//modify by ff  RETURN_IF_ERROR(exec_env_->rpc_mgr_->Init(exec_env_->krpc_address_));
//modify by ff  exec_env_->control_svc_.reset(new ControlService(exec_env_->rpc_metrics_));
//modify by ff  RETURN_IF_ERROR(exec_env_->control_svc_->Init());
