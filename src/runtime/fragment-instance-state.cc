...
Status FragmentInstanceState::Exec() {
 ...
 //modify by ff     query_state_->ErrorDuringPrepare(status, instance_id());
    } else {
...
//modify by ff       query_state_->ErrorDuringExecute(status, instance_id());
...
/*modify by ff    const StatefulStatusPB& stateful_report =
...
//modify by ff    RETURN_IF_ERROR(fragment_state_->InvokeCodegen(event_sequence_));
  }
