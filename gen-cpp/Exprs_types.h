...
typedef struct _TExprNode__isset {
  _TExprNode__isset() : fn(false), vararg_start_idx(false), bool_literal(false), case_expr(false), date_literal(false), float_literal(false), int_literal(false), in_predicate(false), is_null_pred(false), literal_pred(false), slot_ref(false), string_literal(false), tuple_is_null_pred(false), decimal_literal(false), agg_expr(false), timestamp_literal(false), kudu_partition_expr(false), cast_expr(false) {}
  bool fn :1;
...
class TExprNode {
 public:
  ...
  TExprNode() : node_type((TExprNodeType::type)0), num_children(0), is_constant(0), vararg_start_idx(0) {
  }
